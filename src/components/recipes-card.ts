...
export class MealieRecipeCard extends MealieBaseCard {
  ...
  private _visibleFavoriteRecipes(): MealieRecipe[] {
    ...
    return cached.filter((r) => this._favorites.get(r.slug) !== false);
  }
  ...
  private _filteredRecipes(): MealieRecipe[] {
    if (this.config.show_categories && !this._selectedCategory) return [];

    let filtered = this.recipes;

    const query = this._searchQuery.trim().toLocaleLowerCase(this.hass?.locale?.language ?? 'fr');
    if (query) {
      filtered = filtered.filter((recipe) => recipe.name?.toLocaleLowerCase(this.hass?.locale?.language ?? 'fr').includes(query));
    }

    if (this._selectedCategory) {
      filtered = filtered.filter((recipe) => recipe.categories?.some((category) => category.slug === this._selectedCategory));
    }

    return filtered.slice(0, this.config.result_limit ?? DEFAULT_RESULT_LIMIT);
  }
        ...
        ${showCategories
          ? html`<select
            ...
            >
              ...
              ${this._categories().map((category) => html`<option .value=${category.slug}>${category.name}</option>`)}
            </select>`
          : nothing}
        ${showImport
          ? html`<ha-icon-button
